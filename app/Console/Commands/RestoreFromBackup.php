...
class RestoreFromBackup extends Command
{
  ...
  public function handle()
  {
    ...
    for ($i = 0; $i < $za->numFiles; $i++) {
      ...
      $good_extensions = [
        'png', 'gif', 'jpg', 'svg', 'jpeg', 'doc', 'docx', 'pdf', 'txt',
    ...
    $this->info("$mysql_binary -h " . escapeshellarg(config('database.connections.mysql.host')) . ' -u ' . escapeshellarg(config('database.connections.mysql.username')) . ' ' . escapeshellarg(config('database.connections.mysql.database')));


    $proc_results = proc_open(
      "$mysql_binary -h " . escapeshellarg(config('database.connections.mysql.host')) . ' -u' . (config('database.connections.mysql.username')) . ' -p' . (config('database.connections.mysql.password')) . ' ' . (config('database.connections.mysql.database')), // yanked -p since we pass via ENV
    ...
    $setting->header_color = '#00b140';
    $setting->skin = 'orange';
    $setting->saml_enabled = 0;
    $setting->saml_sp_privatekey = '';
    $setting->save();
